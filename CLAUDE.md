...
--udp-migrate-egfx        # EXPERIMENTAL, opt-in (default OFF; requires
                          ...
                          #   FEATURE ONLY: under packet loss the reliable ordered
                          #   tunnel head-of-line-blocks like TCP and, once the
                          #   client abandons it, EGFX freezes with no recovery until
                          #   reconnect (audio survives on TCP). Promoted 2026-06-28
                          #   from the MACRDP_UDP_MIGRATE_EGFX env var (still works as
                          #   a fallback). macOS-built; protocol layer cross-platform.
