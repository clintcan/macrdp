...
v0 — daily-driver usable on a trusted LAN, and usable **over the internet** (VPN / ZeroTier / high-latency links, including mobile). **Latest release: [v0.9.2](https://github.com/clintcan/macrdp/releases/latest)** — *blank-recovery clean-presentation latch*: a point release fixing a v0.9.1 regression where the mstsc/Windows-App reconnect-blank detector force-dropped a **working** session — one that presents the desktop fine but reports zero decode+render time — into an auto-reconnect loop. A durable clean-presentation latch now disarms the detector for any session that presents at connect, while a genuine reconnect-blank still self-heals (both live-verified). The default runtime path is unchanged. Builds on **v0.9.1** (*the lockable-headless release*: the opt-in **`--shield-primary`** headless blanking mode that keeps the Mac lockable, client-resolution auto-adopt on `--virtual-display`, and a `--detach-primary` launchd-restart stopgap for the macOS-26 panel-re-enable bug) and **v0.9.0** (*the webcam release*: a client webcam presents as a **real macOS camera** via `--enable-camera-redirection` — as far as is known the first known open-source RDP _server_ to do so; H.264 over MS-RDPECAM → VideoToolbox decode → a CoreMediaIO Camera system extension, live-verified at 1080p/~30 fps), v0.8.40 (the *headless-laptop release*), and v0.8.39 (the *smooth-resize release*).
